...
source = imread('Img1.jpg');
destination = imread('Img2.jpg');

S = 1;  % sigma = 1-3
N = 3;  % size of local NxN neighborhood
D = 20;  % radius of neighboor
Ms = 90;    % number of corners we want to detect in source
Md = 100;    % number of corners we want to detect in destination
L = 7;  % size of patche = 2L + 1
...
N = 200;    % the number of iterations required
d = 1;  % the threshold used to identify a point that fits well
T = 5;  % the number of nearby points required to assert a model fits well
