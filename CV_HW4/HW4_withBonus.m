...
figure(1); imshow(img1, 'InitialMagnification', 150); title('img1');
% [x1, y1] = ginput(N); 
hold on;
% plot(x1, y1, 'ys');
for i = 1:N
...
figure(2); imshow(img2, 'InitialMagnification', 150); title('img2');
% [x2, y2] = ginput(N); 
hold on;
% plot(x2, y2, 'ys');
for i = 1:N
...
[rows1 cols1 tmp] = size(img1);
[rows2 cols2 tmp] = size(img2);
[x1 y1 T1 S1] = preConditioning(x1, y1, rows1, cols1, N);
[x2 y2 T2 S2] = preConditioning(x2, y2, rows2, cols2, N);
...
% prove with preconditioning step
h1 = inv(T2) * inv(S2) * h1 * S1 * T1;
h2 = inv(T2) * inv(S2) * h2 * S1 * T1;
x1 = img1_points(:,1);
y1 = img1_points(:,2);
x2 = img2_points(:,1);
y2 = img2_points(:,2);
